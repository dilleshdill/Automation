...
const BidderAuctionDetailes = () => {
  ...
  const handlePlaceBid = async () => {
    if (!bidAmount || parseFloat(bidAmount) <= (auction.current_bid || auction.starting_bid)) {
      alert(`Bid must be higher than current bid: $${auction.current_bid || auction.starting_bid}`);
      return;
    }

    try {
      const res = await axios.post(
        `${DOMAIN}/auction/place-bid`,
        {
          auction_id: id,
      ...
      if (res.status === 200) {
        ...
        const updatedAuction = await axios.post(
          `${DOMAIN}/auction/get-auction`,
          { auction_id: id },
          { withCredentials: true }
        );
        setAuction(updatedAuction.data?.existingAuction || {});
      }
    } catch (err) {
  ...
  if (loading) {
    return (
      <>
        <BidderHomeNavBar />
        <div className="min-h-screen relative overflow-x-hidden bg-gradient-to-b from-slate-900 via-green-900/30 to-slate-900 flex items-center justify-center">
          <div className="text-center">
            ...
            <p className="mt-4 text-white/70">Loading auction details...</p>
          </div>
        </div>
      </>
    );
    ...
    <>
        ...
        <div className="relative z-10 px-6 py-10 flex justify-center">
          ...
          >
            ...
            <div className="relative backdrop-blur-md bg-gradient-to-br from-white/10 to-white/5 border border-white/20 rounded-2xl shadow-2xl overflow-hidden hover:shadow-yellow-400/20 p-8">
              ...
              <button
                onClick={() => navigate(-1)}
                className="absolute top-4 left-4 z-20 text-white/70 hover:text-white transition-colors"
              >
              ...
              {/* HEADER + STATUS */}
              <div className="flex flex-col md:flex-row items-start justify-between gap-3 relative z-10 mt-8 md:mt-0">
              ...
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-10 mt-10 relative z-10">
                ...
                <div className="flex justify-center">
                  <div className="relative group/image">
                    {auction?.auction_img ? (
                      ...
                      <div className="w-72 h-80 bg-black/50 rounded-xl border-2 border-white/20 flex items-center justify-center text-white/50">
                ...
                <div className="lg:col-span-2 space-y-5">
                  <div className="p-5 bg-black/30 rounded-lg border border-white/10 space-y-4">
                    ...
                    <Detail label="Player Time" value={auction?.auction_time ? `${auction.auction_time}s` : '-'} />
                    <Detail label="Status" value={auction?.status} />
                    {auction?.starting_bid && (
                      <Detail label="Starting Bid" value={`$${Number(auction.starting_bid).toLocaleString()}`} />
                    )}
                    {auction?.current_bid && (
                      <Detail label="Current Bid" value={`$${Number(auction.current_bid).toLocaleString()}`} />
                    )}
                    {auction?.bid_count !== undefined && (
                      <Detail label="Total Bids" value={auction.bid_count} />
                    )}
                    {auction?.end_time && (
                      <Detail label="Ends In" value={new Date(auction.end_time).toLocaleString()} />
                    )}
                  </div>
                  ...
                  <div className="flex flex-wrap gap-4 pt-4">
                    {auction?.status === 'live' && (
                      <button
                        ...
                        className="px-8 py-3 bg-gradient-to-r from-green-600 to-green-700 hover:from-green-700 hover:to-green-800 text-white font-semibold rounded-xl shadow-lg hover:shadow-xl transition duration-200 transform hover:-translate-y-0.5"
                      >
                        Place Bid
                      </button>
                    )}
                    {auction?.status === 'upcoming' && (
                      <button className="px-8 py-3 bg-gradient-to-r from-blue-600 to-blue-700 hover:from-blue-700 hover:to-blue-800 text-white font-semibold rounded-xl shadow-lg hover:shadow-xl transition duration-200 transform hover:-translate-y-0.5">
                        Set Reminder
                      </button>
                    )}
                    <button className="px-8 py-3 bg-gradient-to-r from-slate-600 to-slate-700 hover:from-slate-700 hover:to-slate-800 text-white font-semibold rounded-xl shadow-lg hover:shadow-xl transition duration-200 transform hover:-translate-y-0.5">
                      Add to Watchlist
                    </button>
                  </div>
                </div>
      ...
      {showBidModal && (
        <div className="fixed inset-0 bg-black bg-opacity-70 flex items-center justify-center p-4 z-50 backdrop-blur-sm">
          <div className="bg-gradient-to-b from-slate-800 to-slate-900 rounded-2xl max-w-md w-full p-6 border border-yellow-400/30 shadow-2xl">
            <h3 className="text-xl font-bold text-transparent bg-clip-text bg-gradient-to-r from-yellow-400 to-orange-400 mb-4">
              Place Your Bid
            </h3>
            
            <div className="mb-4 space-y-2">
              <p className="text-sm text-white/70">Current Bid: 
                <span className="font-semibold text-green-400 ml-2">
                  ${Number(auction.current_bid || auction.starting_bid).toLocaleString()}
                </span>
              </p>
              <p className="text-sm text-white/70">Minimum Bid: 
                <span className="font-semibold text-blue-400 ml-2">
                  ${(Number(auction.current_bid || auction.starting_bid) + 1).toLocaleString()}
                </span>
              </p>
            </div>
            ...
            <input
              ...
              className="w-full px-4 py-3 bg-white/10 border border-white/20 rounded-xl focus:outline-none focus:ring-2 focus:ring-yellow-400 text-white placeholder-white/50 mb-4"
              min={(auction.current_bid || auction.starting_bid) + 1}
              step="0.01"
            />
            ...
            <div className="flex gap-3">
              <button
                ...
                className="flex-1 px-4 py-3 bg-gradient-to-r from-green-600 to-green-700 hover:from-green-700 hover:to-green-800 text-white font-semibold rounded-xl transition duration-200"
              >
                Confirm Bid
              </button>
              <button
                ...
                className="flex-1 px-4 py-3 bg-white/10 hover:bg-white/20 text-white font-semibold rounded-xl transition duration-200"
              >
                Cancel
              </button>
            </div>
