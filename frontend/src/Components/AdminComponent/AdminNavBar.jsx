...
const AdminNavBar = () => {
  ...
  useEffect(() => {
    const fetchLiveCount = async () => {
      try {
        const response = await axios.get(DOMAIN + "/admin/live-auction-count", {
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex h-16 items-center justify-between">
          ...
          <div className="flex items-center gap-2 cursor-pointer" onClick={() => navigate("/admin")}>
            ...
            <div>
              <h2 className="text-lg font-serif font-bold text-amber-900">Admin Panel</h2>
              <p className="text-[10px] text-amber-600 -mt-1">CONTROL CENTER</p>
            </div>
          </div>
          ...
          <div className="flex items-center gap-3 relative">
            ...
            {profileOpen && (
              <div className="absolute right-0 top-14 w-48 bg-white rounded-lg shadow-xl border border-amber-100 py-2 animate-fade">
                ...
                <button
                  disabled
                  className="block w-full text-left px-4 py-2 text-sm text-gray-400 hover:bg-amber-50 cursor-not-allowed"
                >
                  Settings
                </button>
      ...
      {open && (
        <div className="md:hidden bg-white border-t border-amber-100 shadow-lg">
          <div className="px-4 py-3 space-y-2">
            <Link
              to="/admin"
              className="block py-2 text-gray-700 hover:text-amber-700 transition"
